...
contract Zeta is ERC721, Ownable, Pausable {
    ...
    function init_mint(uint256 _uint) external onlyOwner {
        _mint(_uint);
    }
    ...
    function mint(uint256 tokenQuantity) external payable whenNotPaused {
        ...
        require(total_supply + tokenQuantity <= max_supply, 'Already achieve max supply');
        require(_isSaleActive, 'Sale must be active to mint');
        require(tokenQuantity * mintPrice <= msg.value, 'Not enough ether to mint');
